...
| Step | Output | Use |
| --- | --- | --- |
| format / test / clippy / Rust 1.88 check | job result | Rust workspace の基本的な regression と MSRV compile を見る。 |
| `cargo audit` | dependency-audit job result | lockfileに既知のRustSec advisoryがないかを見る。 |
...
| Step | Output | Use |
| --- | --- | --- |
| format / test / clippy / Rust 1.88 check | job result | Review basic Rust workspace regression and MSRV compilation. |
| `cargo audit` | dependency-audit job result | Check the lockfile for known RustSec advisories. |
