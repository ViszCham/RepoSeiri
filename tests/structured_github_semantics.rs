...
#[test]
fn workflow_keeps_dynamic_values_permissions_and_action_refs_typed() {
...
#[test]
fn issue_forms_preserve_unknown_fields_and_static_route_candidates() {
...
#[test]
fn codeowners_compiles_supported_patterns_and_retains_skipped_lines() {
...
#[test]
fn dependency_and_scope_coverage_remain_static_and_bounded() {
    ...
    assert!(markdown.contains("Structured GitHub Semantics"));
    assert!(markdown.contains("do not establish workflow success"));
    let query = seiri_report::codex_query_repository_to_json(
        repo.path(),
        ProfileKind::Common,
        seiri_core::AnalysisScope::Repository,
        seiri_report::CodexQueryKind::Governance,
    )
    .expect("documents query");
    assert!(query.contains("\"github\""));
}

#[test]
fn dynamic_dependency_values_never_become_literals() {
...
fn workflow(snapshot: &seiri_core::RepositoryAnalysis) -> &seiri_core::Workflow {
...
fn issue_form(snapshot: &seiri_core::RepositoryAnalysis) -> &seiri_core::IssueForm {
