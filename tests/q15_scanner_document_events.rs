...
#[test]
fn q15_bounded_walker_separates_ignore_and_classification() {
...
#[test]
fn q15_document_events_keep_utf8_spans_and_soft_diagnostics() {
...
#[test]
fn q15_document_limits_and_invalid_utf8_are_typed_failures() {
...
#[test]
fn q15_document_scan_wire_rejects_missing_bounds_and_order() {
    let source = "# Docs\n\n# Security\n";
    let document = seiri_markdown::scan_document("README.md", source).expect("scan document");
...
fn q15_audit_uses_document_events_as_canonical_evidence_input() {
    ...
    let summary = snapshot.readme.as_ref().expect("README summary");
    ...
    assert!(document.events().iter().all(|event| {
        ...
        snapshot
            ...
            .facts()
            .iter()
            .any(|fact| fact.span == span && event_matches_fact(event, fact.kind))
    }));
    ...
    assert!(value["readme"].is_object());
}

fn event_matches_fact(event: &DocumentEvent, kind: EvidenceKind) -> bool {
    matches!(
        (event, kind),
        (DocumentEvent::Heading(_), EvidenceKind::MarkdownHeading)
            | (DocumentEvent::Link(_), EvidenceKind::MarkdownLink)
            | (DocumentEvent::Badge(_), EvidenceKind::MarkdownBadge)
            | (
