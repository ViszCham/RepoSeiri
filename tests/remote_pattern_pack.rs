...
#[test]
fn remote_adapter_is_opt_in_read_only_and_never_serializes_tokens() {
...
#[test]
fn remote_adapter_keeps_terminal_states_distinct_on_the_canonical_wire() {
    ...
    assert!(wire.get("remote_evidence").is_some());
}

#[test]
fn pattern_pack_requires_all_fixture_classes_and_uses_a_conditional_denominator() {
    ...
    let run = seiri_calibration::calibrate_local_dataset_with_pattern_pack(&dataset, &cli_pack)
        .expect("calibrate local dataset");
...
fn dataset_fixture() -> BenchmarkDataset {
    BenchmarkDataset {
        schema_version: seiri_core::CALIBRATION_SCHEMA_VERSION.to_string(),
...
impl TempRepo {
    fn new(label: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "reposeiri-remote-pattern-{label}-{}-{nonce}",
            std::process::id()
        ));
