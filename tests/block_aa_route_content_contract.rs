use seiri_core::{
    route_content_contract_v2, ClaimBoundaryKind, DocumentDiagnosticKind, DocumentEvent,
...
fn registry_covers_all_routes_and_claim_boundaries() {
    let registry = route_content_contract_v2();
...
fn audit_builds_structural_pair_and_separate_content_priorities() {
    ...
    assert_eq!(snapshot.route_content_v2.assessments.len(), 63);
    assert_eq!(snapshot.route_content_v2.structural_pairs.len(), 1);
    assert!(snapshot.route_content_v2.structural_pairs[0].candidate_only);
    ...
    let compatibility_json = seiri_report::to_json(&snapshot).expect("json");
    assert!(!compatibility_json.contains("route_content_v2"));
    let markdown = seiri_report::to_markdown(&snapshot);
    assert!(markdown.contains("Route Content Contract v2"));
...
fn invalid_utf8_and_event_budget_become_unknown_not_absent() {
    ...
    assert!(invalid.route_content_v2.assessments.iter().any(|item| {
        matches!(
            item.observation,
    ...
    assert!(limited.route_content_v2.assessments.iter().any(|item| {
        matches!(
            item.observation,
