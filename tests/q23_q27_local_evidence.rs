use seiri_core::{
    ContentObservation, CoverageIncompleteReason, CoverageScope, CoverageStatus, DocumentRole,
    DocumentScanStatus, Observation, ProfileKind, RouteContentAtom, UnknownReason,
};
...
#[test]
fn q24_document_index_bounds_scan_budget_and_preserves_role_coverage() {
...
#[test]
fn q25_document_parse_failure_is_indexed_as_unknown_coverage() {
...
#[test]
fn q23_partial_report_coverage_never_converts_missing_content_to_absence() {
    ...
    let docs_concept = snapshot
        .route_content
        .iter()
        .find(|route| route.route == seiri_core::RouteKind::Docs)
        .and_then(|route| route.observation(RouteContentAtom::DocsConceptGuide))
        .expect("docs concept observation");
    assert_eq!(
        docs_concept,
        &ContentObservation::Unknown(UnknownReason::LimitExceeded)
    );
}

#[test]
fn q27_route_content_keeps_presence_separate_from_adequacy_and_off_wire() {
    ...
    assert_eq!(snapshot.route_content.len(), 14);
    assert!(snapshot
        .route_content
        .iter()
        .all(|route| route.atoms.len() == 2));
    for route in &snapshot.route_content {
        for atom in &route.atoms {
            assert!(
                matches!(atom.observation, ContentObservation::Present { .. }),
                "expected {atom:?} to be observed"
            );
        }
    }
    assert!(snapshot
        .route_content
        .iter()
        .flat_map(|route| route.atoms.iter())
        .any(|atom| atom.atom == RouteContentAtom::SecurityDisclosureChannel));
    ...
    assert!(wire.get("document_index").is_none());
    assert!(wire.get("route_content").is_none());
}
