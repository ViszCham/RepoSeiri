...
fn streaming_jsonl_matches_materialized_results_for_unique_records() {
    ...
    let materialized = seiri_calibration::calibrate_dataset(&dataset);
    ...
    assert_eq!(
        ...
        CalibrationAggregationMode::MaterializedCompatibility
    );
}
...
#[test]
fn calibration_run_without_q17_trace_reads_with_compatibility_defaults() {
    let dataset =
    ...
    let legacy = serde_json::from_value::<seiri_core::CalibrationRun>(value)
        .expect("legacy calibration run");
    assert_eq!(
        legacy.resource_trace.aggregation_mode,
        CalibrationAggregationMode::MaterializedCompatibility
    );
    assert_eq!(legacy.resource_trace.records_seen, 0);
    assert!(legacy.resource_trace.replay_digest.is_none());
}
