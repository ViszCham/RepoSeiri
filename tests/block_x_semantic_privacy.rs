...
#[test]
fn x4_x5_related_targets_are_relations_not_conflicts() {
    let snapshot = seiri_report::audit_repository_subtree(fixture("block-x-related-targets"))
...
#[test]
fn x5_two_canonical_targets_remain_a_true_conflict() {
    let snapshot = seiri_report::audit_repository_subtree(fixture("block-x-true-conflict"))
...
#[test]
fn x5_reposeiri_self_audit_has_no_false_document_conflicts() {
...
#[test]
fn x1_x3_standard_audit_has_typed_profile_semantics_without_priors() {
    ...
    assert!(profile.branches.iter().all(|branch| {
        branch.prior_x1000 == 0
            && branch.semantics.calibration_prior == CalibrationPriorState::NotRequested
            && branch.confidence_x100 == branch.semantics.rank_score.get()
            && branch.evidence_score_x100 == branch.semantics.evidence_match.get()
            && branch.score_x100 == branch.semantics.fit.get()
    }));
    let json = serde_json::to_string(profile).expect("compatibility profile JSON");
    assert!(!json.contains("semantics"));
    assert!(!json.contains("calibration_prior"));
    let roundtrip: seiri_core::ProfileReport =
        serde_json::from_str(&json).expect("compatibility profile roundtrip");
    assert!(roundtrip.branches.iter().all(|branch| {
        branch.semantics.fit.get() == branch.score_x100
            && branch.semantics.evidence_match.get() == branch.evidence_score_x100
            && branch.semantics.rank_score.get() == branch.confidence_x100
            && branch.semantics.calibration_prior == CalibrationPriorState::NotRequested
    }));
    assert!(profile
        .branch_summary
...
#[test]
fn x2_x6_explicit_local_prior_never_reaches_public_surfaces() {
    ...
    let body = serde_json::json!({
        ...
        "private_note": "BLOCK_X_PRIVATE_BODY_SENTINEL",
        "priors": [
            {
    ...
    assert_eq!(library.prior_x1000, 0);
    assert_eq!(
    ...
    let context = seiri_codex::build_review_kernel(&snapshot, &plan, None).compatibility_v1();
    let surfaces = [
...
#[test]
fn x6_local_prior_parse_errors_redact_path_and_invalid_values() {
    let temp = temporary_pack_path();
    let fingerprint = seiri_patterns::common_pattern_pack()
        .fingerprint()
        .to_string();
    let private_value = "BLOCK_X_PRIVATE_INVALID_ROUTE_SENTINEL";
    let body = serde_json::json!({
        "schema_version": seiri_calibration::LOCAL_PRIOR_SCHEMA_VERSION,
        "registry_fingerprint": fingerprint,
        "priors": [{
...
#[test]
fn x2_local_prior_loader_rejects_invalid_counts_fingerprint_and_duplicates() {
...
fn temporary_pack_path() -> PathBuf {
    ...
    let directory = std::env::temp_dir().join(format!(
        "reposeiri-block-x-private-sentinel-{}-{nonce}",
        std::process::id()
    ));
