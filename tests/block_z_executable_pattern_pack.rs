...
#[test]
fn block_z_loads_and_executes_all_group_fixture_classes_without_process_or_network() {
...
#[test]
fn block_z_loader_rejects_unbounded_paths_invalid_predicates_and_partial_absence() {
    let fixture = BlockZFixture::new();
...
#[test]
fn block_z_compares_evidence_ids_and_requires_review_before_adoption() {
    let fixture = BlockZFixture::new();
    ...
    let review = PatternAdoptionReview {
        review_id: "review-block-z".to_string(),
...
fn block_z_private_overlay_applies_locally_without_public_prior_values() {
    ...
    for output in [&json, &markdown] {
        assert!(!output.contains("BLOCK_Z_PRIVATE_SENTINEL"));
...
impl BlockZFixture {
    fn new() -> Self {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("reposeiri-block-z-{}-{nonce}", std::process::id()));
