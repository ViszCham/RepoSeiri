...
| Codex native v2 JSON | `cargo run --quiet -p seiri-cli -- codex --path . --profile library --schema native-v2 --format json` |
| bounded query | `cargo run --quiet -p seiri-cli -- codex --path . --profile library --view query --query summary --format json` |
| wording linter context | `cargo run --quiet -p seiri-cli -- codex --path . --profile library --view linter --format markdown` |
